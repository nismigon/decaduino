...
// States
enum {STATE_IDLE, STATE_SEND, STATE_WAIT, STATE_CALCULATION};

// Delay before each ranging
#define RANGING_DELAY 1000

// Group Number
#define GROUP_NUMBER 37
...
typedef struct {
  ...
  uint64_t t3;
  uint64_t t4;
} TIME_EMISSION_RECEPTION;

TIME_EMISSION_RECEPTION times[NODE_NUMBER];
...
void loop()
{
  switch (state) {
    case STATE_IDLE:
      ...
      delay(RANGING_DELAY);
      // RAZ values
      reset_table_times(times, NODE_NUMBER);
      state = STATE_SEND;
      break;
    case STATE_SEND:
...
void reset_table_times(TIME_EMISSION_RECEPTION *times, int numbers) {
